...
def error_handler(exc_info):
    """Error handler for integrating with Report Portal system."""
    traceback.print_exception(*exc_info)
    core_log.exception("Got an exception when integrating with Report Portal")
...
def get_configuration():
    ...
    if "reportportal" not in content:
        raise ValueError("No 'reportportal' key is defined in {}".format(
            config_file))
...
class ReportPortalHandler(AbstractResultHandler):
    ...
    def start_test(self, test):
        ...
        if isinstance(test, TestFlow):
            ...
            item_type = "STEP"
        elif isinstance(test, TestBlock):
            item_type = "STEP"
            description = "|{}| {}".format(self.MODE_TO_STRING[test.mode],
                                           description)
        else:
            raise TypeError("Illegal value {} of class {}".format(test,
                                                                  type(test)))
